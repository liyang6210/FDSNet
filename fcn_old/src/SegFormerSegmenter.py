...
class SegFormer(nn.Module):
    def __init__(self, num_classes=104, phi='b5', pretrained=False):
        ...
        self.mask_transformer0 = MaskTransformer(
            num_classes,
            64,
            768,
            12,
        ...
        self.mask_transformer1 = MaskTransformer(
            num_classes,
            128,
            768,
            12,
        ...
        self.mask_transformer2 = MaskTransformer(
            num_classes,
            320,
            768,
            12,
        ...
        self.mask_transformer3 = MaskTransformer(
            num_classes,
            512,
            768,
            12,
        ...
        self.upsample = Upsample(640, (16,16))
        self.scale = 768 ** -0.5
        self.layer0=nn.Conv2d(64,128,kernel_size=3,padding=1)
        # self.layer1=nn.Conv2d(128,104,kernel_size=3,padding=1)
        self.layer2=nn.Conv2d(320,128,kernel_size=3,padding=1)
        self.layer3=nn.Conv2d(512,128,kernel_size=3,padding=1)


    # def forward(self, inputs):
    #     print("inputs",inputs.shape)
    #     B,H, W = inputs.size(0),inputs.size(2), inputs.size(3)
    #
    #     x = self.backbone.forward(inputs)
    #     print(x[0].shape,x[1].shape,x[2].shape,x[3].shape)
    #     z0, c0 = self.mask_transformer0(x[0])
    #     z1, c1 = self.mask_transformer1(x[1])
    #     z2, c2 = self.mask_transformer2(x[2])
    #     z3, c3 = self.mask_transformer3(x[3])
    #     print('--------')
    #     masks0 = z0 @ c0.transpose(1, 2)
    #     masks1 = z1 @ c1.transpose(1, 2)
    #     masks2 = z2 @ c2.transpose(1, 2)
    #     masks3 = z3 @ c3.transpose(1, 2)
    #     print('0',masks0.shape,masks1.shape,masks2.shape,masks3.shape)
    #     masks0 = torch.softmax(masks0 / self.scale, dim=-1)
    #     masks1 = torch.softmax(masks1 / self.scale, dim=-1)
    #     masks2 = torch.softmax(masks2 / self.scale, dim=-1)
    #     masks3 = torch.softmax(masks3 / self.scale, dim=-1)
    #     print('1',masks0.shape,masks1.shape,masks2.shape,masks3.shape)
    #
    #     # masks0 = self.upsample(masks0)
    #     masks0=masks0.reshape(B,160,160,-1).permute(0,3,1,2)
    #     masks1=masks1.reshape(B,80,80,-1).permute(0,3,1,2)
    #     masks2=masks2.reshape(B,40,40,-1).permute(0,3,1,2)
    #     masks3=masks3.reshape(B,20,20,-1).permute(0,3,1,2)
    #     print('2',masks0.shape,masks1.shape,masks2.shape,masks3.shape)
    #
    #     masks1 = F.interpolate(masks1, scale_factor=2, mode="bilinear", align_corners=True)
    #     masks2 = F.interpolate(masks2, scale_factor=4, mode="bilinear", align_corners=True)
    #     masks3 = F.interpolate(masks3, scale_factor=8, mode="bilinear", align_corners=True)
    #     print('3',masks0.shape,masks1.shape,masks2.shape,masks3.shape)
    #
    #     result = masks0+masks1+masks2+masks3
    ...
    def forward(self, inputs):
        ...
        # masks0 = self.upsample(masks0)
        x[0]=x[0].reshape(B,160,160,-1).permute(0,3,1,2)
        x[1]=x[1].reshape(B,80,80,-1).permute(0,3,1,2)
        x[2]=x[2].reshape(B,40,40,-1).permute(0,3,1,2)
        x[3]=x[3].reshape(B,20,20,-1).permute(0,3,1,2)
        print('2',x[0].shape,x[1].shape,x[2].shape,x[3].shape)

        x[1] = F.interpolate(x[1], scale_factor=2, mode="bilinear", align_corners=True)
        x[2] = F.interpolate(x[2], scale_factor=4, mode="bilinear", align_corners=True)
        x[3] = F.interpolate(x[3], scale_factor=8, mode="bilinear", align_corners=True)
        print('3',x[0].shape,x[1].shape,x[2].shape,x[3].shape)

        x[0]=self.layer0(x[0])
        # x[1]=self.layer1(x[1])
        x[2]=self.layer2(x[2])
        x[3]=self.layer3(x[3])
        print('4',x[0].shape,x[1].shape,x[2].shape,x[3].shape)
        result=x[0]+x[1]+x[2]+x[3]
        result = result.reshape(B,128,-1).permute(0,2,1)
        print('result',result.shape)
        z,c = self.mask_transformer1(result)
        print('result1',z.shape,c.shape)
        mask = z @ c.transpose(1, 2)
        mask = torch.softmax(mask / self.scale, dim=-1)
        print(mask.shape)
        mask = mask.reshape(B,160,160,-1).permute(0,3,1,2)
        print(mask.shape)
