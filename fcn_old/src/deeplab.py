...
class Backbone1(nn.Module):
    def __init__(self):
        ...
        self.Global = vit_base_patch16_siglip_512()
        self.Local = vit_base_patch16_siglip_256()
        self.GlobalMLA = MLA()
        self.LocalMLA = MLA()
        self.GLA = RefinementAppNet(n_classes=104,inputsize=[512,512])
    def forward(self, x):
        Global = self.Global(x)
        print(Global[0].shape, Global[1].shape, Global[2].shape, Global[3].shape)
        ...
        print(local_out[0].shape, local_out[1].shape, local_out[2].shape, local_out[3].shape)
