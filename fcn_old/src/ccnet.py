...
class SegFormer(nn.Module):
    def __init__(self, num_classes=104, phi='b4', pretrained=False):
        ...
        self.changeC = nn.Conv2d(5,3,kernel_size=1)
    def forward(self, inputs):
        glb = rescale_to(inputs, scale_factor=0.5, interpolation='bilinear')
        loc = image2patches(inputs)
        inputs = torch.cat((loc, glb), dim=0)
        print(inputs.shape)
        # inputs = torch.relu(self.changeC(inputs))
