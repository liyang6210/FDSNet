...
class Backbone1(nn.Module):
    ...
    def forward(self, x):
        ...
        # # deepX = self.reshape(deepX[:, 1:, :])
        # # print(shallowX[3].shape,deepX.shape)
        ...
        print(shallowX[3].shape,1111)
        result = self.PuPHead(shallowX[3])
        print(result.shape)
        return result

def SETRPUP(num_classes: int = 21843, has_logits: bool = True):
