...
class RAF(nn.Module):
    ...
    def forward(self, xb, xs):
        ...
        outB = self.linearxb(matrixOut).unsqueeze(2).unsqueeze(3)
        outS = self.linearxs(matrixOut).unsqueeze(2).unsqueeze(3)
...
class Backbone1(nn.Module):
    ...
    def forward(self, x):
        ...
        deepX = self.deepNet(x)

        deepX[0] = deepX[0].permute(0, 2, 3, 1)
        deepX[0] = self.norm(deepX[0])
        deepX[0] = deepX[0].permute(0, 3, 1, 2)
        deepX[1] = F.interpolate(deepX[1], scale_factor=2, mode='bilinear', align_corners=False)
        deepX[1] = self.changeC(deepX[1])
        print(deepX[0].shape,deepX[1].shape)
        # RAFout0 = self.RAF0(deepX[0], deepX[1])
        result = deepX[0] + deepX[1]
...
def MyModelABL(num_classes: int = 21843, has_logits: bool = True):
...
if __name__ == "__main__":
    ...
    SETRNet = MyModelABL(num_classes=104)
