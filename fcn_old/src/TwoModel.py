...
class Backbone1(nn.Module):
    def __init__(self, num_classes,norm_layer: Union[str, Callable] = nn.LayerNorm):
        ...
        self.deepNet = swin_base_patch4_window12_384()
        ...
        # self.RAF0 = RAF(512,1024)
        # self.RAF1 = RAF(512,512)
        # self.RAF2 = RAF(256,512)
        self.changeC0 = nn.Conv2d(1024,512,1)
        self.changeC1 = nn.Conv2d(512, 512, 1)
        self.changeC2 = nn.Conv2d(512, 256, 1)
    ...
    def forward(self, x):
        ...
        tempdeepx1 = self.changeC0(deepX[1])
        tempdeepx1 = F.interpolate(tempdeepx1, scale_factor=2, mode='bilinear',align_corners=False)
        tempdeepx0 = deepX[0] + tempdeepx1
        tempdeepx0 = self.changeC1(tempdeepx0)
        tempdeepx0 = F.interpolate(tempdeepx0, scale_factor=2, mode='bilinear',align_corners=False)
        tempshallowx3 = shallowX[3]+tempdeepx0
        tempshallowx3 = self.changeC2(tempshallowx3)
        tempshallowx3 = F.interpolate(tempshallowx3, scale_factor=2, mode='bilinear',align_corners=False)
        result = tempshallowx3 + shallowX[2]
        # RAFout0 = self.RAF0(deepX[0],deepX[1])
        # RAFout1 = self.RAF1(shallowX[3],RAFout0)
        # result = self.RAF2(shallowX[2],RAFout1)
...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True):
    ...
    model = Backbone1(num_classes=103)
    return model
