...
def SwinB(pretrained=True):
    ...
    if pretrained is True:
        model.load_state_dict(torch.load('./swin_base_patch4_window12_384_22kto1k.pth', map_location='cpu')['model'],
                              strict=False)
...
class MCLM(nn.Module):
    ...
    def forward(self, l, g):
        ...
        print(g_hw_b_c.device,self.g_pos.device,pools.device,self.p_poses.device)
...
class MVANet(nn.Module):
    ...
    def forward(self, x):
        ...
        sideout2 = patches2image(sideout2[:-1])  ####(5,c,h,w) -> (1 c 2h,2w)
...
if __name__ == "__main__":
    ...
    SETRNet = MVANet().to(device)
    img = torch.randn(1, 3, 1024, 1024).to(device)
    preds = SETRNet(img)
    print(len(preds))
    print(preds[0].shape,preds[1].shape,preds[2].shape,preds[3].shape,preds[4].shape)